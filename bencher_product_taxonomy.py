...
def get_max_stats(candidate_pair: Collection[Candidate], truth_pairs: set):
    ...
    for node0 in candidate_pair[0].G:
        if title_regex.match(node0):
            titles0 += 1
    category0 = len(candidate_pair[0].G) - titles0

    titles1 = 0
    for node1 in candidate_pair[1].G:
        if title_regex.match(node1):
            titles1 += 1
    category1 = len(candidate_pair[1].G) - titles1

    matches = titles0 + titles1
    comparisons = category0 * titles1 + category1 * titles0
...
def get_pair_results(
    ...
    m0 = title_regex.match(node_pair[0])
    if m0:
        if m0.group(1) == node_pair[1]:
            matches.add(node_pair)
    else:
        m1 = title_regex.match(node_pair[1])
        if m1 and m1.group(1) == node_pair[0]:
            matches.add(node_pair)

    if ret_size:
        return matches, 1 - len(matches), comparisons, (len_a, len_b)
    return matches, 1 - len(matches), comparisons
...
def get_sim_funcs(truth_pairs: set, recursive, p_sample, seed):
    vocabulary = None
    sim_func = [
        # cs.random_similarity,
        # wrapped_partial(cs.intersect, truth_pairs=truth_pairs),
        wrapped_partial(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--omega", type=float, help="Omega", default=None,
    )
    parser.add_argument(
        "--eta", type=float, help="Eta", default=None,
    )
